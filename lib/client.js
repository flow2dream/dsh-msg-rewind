window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* lifecycle card is redundant noise. Command rows render as
		* [data-chat-flow-kind="command"] with the command name resolved from the
		* conversation snapshot by anchor key.
		* @param sessions - sessions service.
		*/
		function hideRewindCommandRows(sessions) {
			for (const item of document.querySelectorAll("[data-chat-flow-kind=\"command\"]")) {
				...
				const key = item.getAttribute("data-chat-anchor-key");
				if (key === null) continue;
				const name = commandNameForKey(sessions, key);
				if (name !== "rewind" && name !== "undo") continue;
		...
		*/
		function commandNameForKey(sessions, key) {
			try {
				...
				if (node !== void 0 && node?.kind === "command" && typeof node?.data?.name === "string") return node.data.name;
			} catch (error) {
				/* best-effort */
			}
			return void 0;
