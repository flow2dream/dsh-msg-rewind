...
import { replayPendingInbox } from "./inbox-replay.js";
...
/**
* Fold a session log's agent/inbox/spliced records the way a fresh Inbox would
* on construction, and return the net pending messages per target.
*
...
async function truncateSession(ctx, agent, toSeq) {
	...
	//    immediately.
	//
	//    clear() only cancels what is STILL PENDING in the live in-memory
	//    inbox. Messages the agent had already claimed before the cut (their
	//    claim splices are truncated away) are gone from the live inbox, so
	//    clear() appends no cancel for them — yet their INSERT splices
	//    survive in the kept region. Replaying the cut log (as a fresh Inbox
	//    does on construction at session resume / host restart) therefore
	//    leaves them "pending": hasPending stays true, and the api-proxy
	//    pushes those rolled-back messages to clients as a phantom "waiting
	//    to send" queue after the next send or reconnect. Durably cancel
	//    exactly those ghosts (replayPendingInbox below) so the rewritten
	//    file replays to an empty inbox, matching the live one.
	//
	...
	// The live clear() above only durably cancels messages still pending in
	// memory. Messages the agent had ALREADY claimed before the cut (their
	// claim events were truncated) are gone from the live inbox, so clear()
	// appends no cancel for them — yet their insert splices survive in the
	// kept region. Replaying the cut log (as a fresh Inbox does on construction
	// at session resume / host restart) therefore leaves them "pending":
	// hasPending stays true, and the api-proxy pushes those rolled-back
	// messages to clients as a phantom "waiting to send" queue after the next
	// send or reconnect. Durably cancel exactly those ghosts so the rewritten
	// file replays to an empty inbox, matching the live one.
	const ghosts = replayPendingInbox(session.events);
	for (const target of ["next-turn", "next-step"]) {
		const ghostCount = ghosts[target].length;
		if (ghostCount > 0) {
			session.append("agent/inbox/spliced", {
				target,
				start: 0,
				removedCount: ghostCount,
				inserted: [],
				outcome: "canceled"
			});
		}
...
export { apply, inject, name, replayPendingInbox };
